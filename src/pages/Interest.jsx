      ...
      try {
        ...
        if (Array.isArray(postsResponse.data)) {
          const postsWithDetails = await Promise.all(
            postsResponse.data
              .filter((post) => post.interest === interestname) // Only filter posts that are related to sports
              .map(async (post) => {
                const [commentsResponse, reactionsResponse] = await Promise.all([
                  axios.get(`http://localhost:5000/api/comments/post/${post.id}`, {
                    headers: { Authorization: `Bearer ${token}` },
                  }),
                  axios.get(`http://localhost:5000/api/reactions/post/${post.id}`, {
                    headers: { Authorization: `Bearer ${token}` },
                  }),
                ]);
          ...
          {posts.map((post) => (
            <div key={post.id} className="post-card">
              <img
                ...
                src={post.image ? `http://localhost:5000/${post.image.replace(/\\/g, '/')}` : '/default-image.jpg'}
                alt={post.title}
              />
